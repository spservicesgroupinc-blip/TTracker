  ...
  return (
    <div className="min-h-screen text-fb-text font-fb pb-24 md:pb-8">
            ...
            <div className="space-y-4">
                ...
                <div className="grid gap-4 p-4 sm:p-6 md:grid-cols-[1fr,auto] md:items-center">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-fb-text-tertiary">Today's total: ${todayTotal.toFixed(2)}</p>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className={`min-w-44 rounded-2xl px-6 py-4 text-base font-extrabold text-white shadow-fb-lg transition active:scale-[0.98] disabled:cursor-wait disabled:opacity-60 ${
              ...
              <div className="rounded-3xl border border-fb-divider bg-white shadow-fb p-4 sm:p-5">
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-fb-input-border bg-fb-bg px-3 py-3 text-sm font-semibold text-fb-text focus:outline-none focus:ring-2 focus:ring-fb-blue"
                >
              ...
              <div className="flex items-center justify-between border-b border-fb-divider px-4 py-4 sm:px-5">
                <div>
                  ...
                  <p className="text-xs font-medium text-fb-text-tertiary">
                    {selectedJob ? `Focused on ${selectedJob.name}` : 'Select a job to surface tasks'}
                  </p>
                </div>
              ...
              <div className="space-y-3 p-4 sm:p-5">
                {!selectedJob && (
                  <div className="rounded-2xl border border-fb-divider bg-fb-bg px-4 py-6 text-center">
                    ...
                    <p className="mt-1 text-xs font-medium text-fb-text-tertiary">Use the selector on the left, or add a new job.</p>
                  </div>
                )}
                ...
                {selectedJob && visibleTasks.length === 0 && (
                  <div className="rounded-2xl border border-fb-divider bg-fb-bg px-4 py-6 text-center">
                    <p className="text-sm font-bold text-fb-text">
                      {completedTaskCount > 0 && !showCompletedTasks ? 'All open tasks are done' : 'No tasks yet'}
                    </p>
                    <p className="mt-1 text-xs font-medium text-fb-text-tertiary">
                      {completedTaskCount > 0 && !showCompletedTasks
                        ? 'Show completed tasks or add the next action.'
                        : 'Add tasks from the quick action button.'}
                    </p>
                  </div>
                )}
                      ...
                      <div key={task.id} className="rounded-2xl border border-fb-divider bg-white p-3 shadow-fb">
                        ...
                        <div className="mt-3 flex gap-2">
                          <button
                            ...
                            className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-bold transition ${statusMeta.buttonClassName}`}
                          >
      ...
      {showQuickActions && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="absolute inset-x-0 bottom-24 px-4">
            <div className="mx-auto max-w-md rounded-3xl border border-fb-divider bg-white p-3 shadow-fb-xl animate-slide-up">
              <p className="px-2 pb-2 text-xs font-bold uppercase tracking-[0.18em] text-fb-text-tertiary">Quick Add</p>
              <div className="space-y-2">
                <button onClick={() => openComposer('job')} className="w-full rounded-2xl bg-fb-bg px-4 py-4 text-left hover:bg-fb-active-bg">
                  <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-50 text-fb-blue">
                      <BriefcaseIcon />
                    </div>
                    <div>
                      <p className="text-sm font-extrabold text-fb-text">Add Job</p>
                      <p className="text-xs font-medium text-fb-text-tertiary">Create a new site or project scope.</p>
                    </div>
                  </div>
                </button>
                <button
                  ...
                  className="w-full rounded-2xl bg-fb-bg px-4 py-4 text-left hover:bg-fb-active-bg disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-50 text-fb-green">
                      <ChecklistIcon />
                    </div>
                    <div>
                      <p className="text-sm font-extrabold text-fb-text">Add Task</p>
                      <p className="text-xs font-medium text-fb-text-tertiary">
                        {jobs.length === 0 ? 'Create a job first to start a checklist.' : 'Add next actions to the selected job.'}
                      </p>
                    </div>
                  </div>
                </button>
      ...
      <button
        ...
        className={`fixed bottom-[4.7rem] left-1/2 z-50 flex h-16 w-16 -translate-x-1/2 items-center justify-center rounded-full border-4 border-fb-bg shadow-fb-xl transition md:hidden ${
        ...
        <div className="grid grid-cols-3">
          {([
            ...
            <button
              ...
              className={`relative flex flex-col items-center py-2 pt-2.5 transition ${activeTab === key ? 'text-fb-blue' : 'text-fb-text-tertiary'}`}
            >
              {activeTab === key && <span className="absolute left-1/2 top-0 h-[3px] w-10 -translate-x-1/2 rounded-b-full bg-fb-blue" />}
              <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${activeTab === key ? 'bg-sky-50' : ''}`}>
              ...
              <span className={`mt-0.5 text-[10px] ${activeTab === key ? 'font-bold' : 'font-medium'}`}>{label}</span>
            </button>
          ))}
        </div>
      </nav>
